...
Data itself is actually bit outdated, since it's data from candy sales in 2007-2015. I'm not even sure if these popular candies changes every year or not.  Now I'm curious to find data sets that are more recent...  
[CandyStore.com's sales from 2007–2015—focusing on the three months leading up to Halloween](https://public.opendatasoft.com/explore/embed/dataset/state-by-state-favorite-halloween-candy/table/)   
...
![](/post/2018-10-25-popular-halloween-candy-on-us-state-grid-map_files/popular_candy_by_state.png)
...
I've imported data to dataframe using [jsonlite](https://cran.r-project.org/web/packages/jsonlite/index.html) package using fromJSON function from [OpenDataSoft site](https://public.opendatasoft.com/explore/?sort=modified).  There seems to be lots of other interesting data sets! 

I've also harvested population by state from Wikipedia using [rvest](https://cran.r-project.org/web/packages/rvest/) package.


```{r dataimport, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r wide_to_long, fig.height=9, fig.width=11, message=FALSE, warning=FALSE, paged.print=FALSE}
...
If candy was ranked #1 in state, I gave gold colour, if #2, then silver, #3 then bronze.
Sort of like Olympic, I've counted how many metals each candies have gotten to decide most popular candy in US.
...
```{r overall_candy_ranking, echo=TRUE, fig.height=9, fig.width=11, message=FALSE, warning=FALSE, paged.print=FALSE}
...
I recently discovered [geofacet](https://cran.r-project.org/web/packages/geofacet/index.html) package. [Examples of graphs](https://hafen.github.io/geofacet/) you can create with this package looks super fun! I thought it was really neat that I can create my own grid using "Geo Grid Designer"!! 


```{r state_visual, fig.height=24, fig.width=11, message=FALSE, warning=FALSE, paged.print=FALSE}
