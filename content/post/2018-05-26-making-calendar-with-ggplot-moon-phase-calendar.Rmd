...
moon.font <- c(0, LETTERS[1:13], 1, LETTERS[14:26])  ## 0 is New Moon then use A to N, 1 is Full Moon, then use M to Z
...
Moon_2018_df <- Moon_2018 %>% 
  mutate(wkdy = fct_inorder(weekdays(date, abbreviate=T)), ## 2018 worked well because it starts on Monday
         ...
         day = day(date), ## Day
         moon.phase = cut(phase,breaks=seq(0,1,by=1/28), ## I want to assign
                          ordered_result=T, 
                          labels=moon.font,right=F)) %>%
  group_by(mo) %>% 
...
Moon_2018_df %>% ggplot(aes(x=wkdy, y=wkn.mo)) + 
  ...
  geom_text(aes(label=moon.phase), family="Moon Phases", hjust=1, color="white", size=5) +
  facet_wrap(~mo, scales="free_x", ncol=3) +  ## So that Each Line is Quarter!
  scale_x_discrete() +
  scale_y_reverse(breaks=NULL) +
  scale_fill_viridis_c(name="Moon Illumination", option="inferno",  begin=0.2, end=0, guide="none") +  ## so that sky is darkest when there's new moon
  theme_ipsum_rc() +
...
However after creating the above, I realized Full Moon dates and fonts are NOT exactly aligned....
I guess I'd probably need to do better way to asign phase value to the font... 
